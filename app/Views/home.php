...
<body>
    ...
    <div id="carouselExample" class="carousel slide w-75 mx-auto" data-bs-ride="carousel" data-bs-interval="1000">
        <div class="carousel-inner">
            <div class="carousel-item active">
                <img src="<?= base_url('assets/img/campoAlpicat.jpg'); ?>" class="d-block w-100 rounded img-fluid"
                    alt="Campo de fútbol Alpicat" style="height: 500px; object-fit:cover">
            </div>
            <div class="carousel-item">
                <img src="<?= base_url('assets/img/campoAlpicat.jpg'); ?>" class="d-block w-100 rounded img-fluid"
                    alt="Imagen 2" style="height: 500px; object-fit:cover">
            </div>
            <div class="carousel-item">
                <img src="<?= base_url('assets/img/campoAlpicat.jpg'); ?>" class="d-block w-100 rounded img-fluid"
                    alt="Imagen 3" style="height: 500px; object-fit:cover">
            </div>
        </div>
                ...
                <div class="w3-row">
                    ...
                    <div class="calendario-container">
                        ...
                        <div class="calendario">
                            ...
                            <?php $daysInMonth = date('t', strtotime("$year-$month-01"));
                            for ($day = 1; $day <= $daysInMonth; $day++):
                                $isToday = ($day == $today && $year == $currentYear && $month == $currentMonth);
                                ?>
                                <div class='calendario-dia <?= $isToday ? 'hoy' : '' ?>'>
                                    <div><?= $day ?></div>
                                    <?php if (isset($eventDays[$day])): ?>
                                        <div class="event-dot" data-eventos='<?= json_encode($eventDays[$day]) ?>'></div>
                                    <?php endif; ?>
                                </div>
                            <?php endfor; ?>
                        </div>
                        <div id="popup-evento" class="w3-modal" style="display: none;">
                            <div class="w3-modal-content w3-padding w3-card-4 w3-animate-top">
                                <span onclick="document.getElementById('popup-evento').style.display='none'"
                                    class="w3-button w3-display-topright">&times;</span>
                                <p id="texto-evento" class="w3-large"></p>
                            </div>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="w3-third w3-margin-bottom">
                            <div class="w3-card-4 noticies"
                                ...
                                <a href="<?= base_url('noticia/' . esc($noticia['url'])) ?>"
                                    class="w3-button w3-green w3-margin-top w3-margin-bottom w3-margin"><?= lang('home.LlegirMes') ?></a>
                            </div>
                        </div>
                    <?php endforeach; ?>
                </div>
    ...
    <script>
        ...
        document.addEventListener("DOMContentLoaded", function () {
            const puntosEvento = document.querySelectorAll(".event-dot");
            const popup = document.getElementById("popup-evento");
            const texto = document.getElementById("texto-evento");

            puntosEvento.forEach(punto => {
                punto.addEventListener("click", function (e) {
                    e.stopPropagation();
                    const eventos = JSON.parse(this.getAttribute("data-eventos"));
            ...
            popup.addEventListener("click", function (e) {
                const modalContent = popup.querySelector(".w3-modal-content");
