...
<head>
    ...
    <title><?= lang('staff.titolModify') ?></title>
</head>

<body>
    <?= helper('form') ?>
    <form action="<?= base_url('gestio/staff/modify/' . $staff['id']) ?>" method="POST">
        <div class="w3-container w3-card w3-white w3-padding w3-margin w3-round-large" style="max-width: 500px; margin: auto;">
            <h2 class="w3-center"><?= lang('staff.subtitolModifu') ?></h2>

            <p>
                <label for="nom" class="w3-text-black"><strong><?= lang('staff.nom') ?></strong></label>
                <input class="w3-input w3-border w3-round" type="text" id="nom" name="nom" value="<?= $staff['nom'] ?>">
            ...
            <p>
                ...
                <input class="w3-input w3-border imatge" type="text" id="imatge" name="imatge" value="<?= $staff['img'] ?>" readonly>
            ...
            <p>
                ...
                <select class="w3-select w3-border" name="carrec" id="carrec">
                    ...
                    <?php foreach ($carrecs as $carrec): ?>
                        <option value="<?= $carrec['id'] ?>" <?= $staff['id_carrec'] == $carrec['id'] ? 'selected' : '' ?>><?= $carrec['nom'] ?></option>
                    <?php endforeach; ?>
                </select>
            ...
            <p>
                ...
                <textarea class="w3-input w3-border w3-round" id="descripcio" name="descripcio" rows="4"><?= $staff['descripcio'] ?></textarea>
            ...
            <p>
                ...
                <select class="w3-select w3-border" name="estat" id="estat">
                    <option value="" disabled selected><?= lang('staff.seleccionaOpcio') ?></option>
                    <option value="0" <?= $staff['estat'] === '0' ? 'selected' : '' ?>><?= lang('staff.no_publicat') ?></option>
                    <option value="1" <?= $staff['estat'] === '1' ? 'selected' : '' ?>><?= lang('staff.publicat') ?></option>
                </select>
            <div class="error"><?= validation_show_error('estat') ?></div>
            </p>

            <p class="w3-center">
                <button type="submit" class="w3-button w3-green w3-round w3-margin-top"><?= lang('staff.modificar') ?></button>
            </p>
        </div>
    </form>
